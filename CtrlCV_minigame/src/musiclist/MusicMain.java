...
public class MusicMain {
	...
	public void game(String a) {
		...
		musicDTO music8 = new musicDTO("응급실", "응급실", ".\\music\\IZI-응급실(2초).mp3");
		...
		while (true) {
			...
			if (menu == 1) {
				...
				for (int i = 0; i < arr.length; i++) {
					...
					System.out.println();
					System.out.println("                         [1] 문제듣기 [2] 정답쓰기 [3] 힌트 [4] 다음문제듣기");

					String title = ml1.get(index).getTitle();
					String singer = ml1.get(index).getSinger();
					String path = ml1.get(index).getPath();
					...
					if (menu2 == 1) {
						...
						title = ml1.get(index).getTitle();
						singer = ml1.get(index).getSinger();
						path = ml1.get(index).getPath();
						...
						System.out.println("정답은 " + arr[index] + " 였습니다ㅋ");
						index++;
						title = ml1.get(index).getTitle();
						singer = ml1.get(index).getSinger();
						path = ml1.get(index).getPath();
